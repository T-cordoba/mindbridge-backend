...
const messageValidation = [
  body('content')
    ...
    .isLength({ max: 4000 }).withMessage('Message exceeds 4000 characters'),
];
...
export default (controller: JournalController): Router => {
  const router = Router();
   ...
   *     responses:
   *       200:
   *         description: "Paginated sessions: { data: Session[], meta: { total, page, limit, totalPages } }"
   */
  router.get('/sessions', controller.list);

  /**
   * @swagger
   * /v1/journal/sessions/{id}:
   *   get:
   *     tags: [Journal]
  ...
  /**
   * @swagger
   * /v1/journal/sessions/{id}/messages:
   *   post:
   *     tags: [Journal]
